...
public class JPOSSampler extends AbstractSampler {
    ...
    public JPOSSampler() {
        LOGGER.info("JPOSSampler()");
    }

    public void initialize() {
        LOGGER.info("initilalize()");
        processPackagerFile();
        processDataRequest();

        if (customPackager == null) {
            LOGGER.info("Undefined packager");
            return;
        }

        LOGGER.info("packager available");
    ...
    protected void processDataRequest() {
        String reqFile = getPropertyAsString(REQUEST);
        if (reqFile == null) {
            LOGGER.info("Undefined data request");
            return;
        }
    ...
    public void processPackagerFile() {
        ...
        if (packagerFile == null) {
            LOGGER.info("Undefined packager file");
            return;
        }

        File initialFile = new File(packagerFile);
        if (!initialFile.exists()) {
            LOGGER.info("Packager file not exists");
            return;
        }
        try {
